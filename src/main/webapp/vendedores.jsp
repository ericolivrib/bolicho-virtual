...
<section>
    <form action="vendedores" method="POST">
        ...
        <input type="number" min="0" name="numero_casa" value="${vendedor.usuario.endereco.numeroCasa}" required
               placeholder="Número"><br>
        <select name="bairro" required>
            <c:choose>
                <c:when test="${logica == 'AtualizarVendedor'}">
                    <option selected
                            value="${vendedor.usuario.endereco.bairro}">${vendedor.usuario.endereco.bairro}</option>
                </c:when>
                <c:otherwise>
        ...
        <input type="text" name="complemento" value="${vendedor.usuario.endereco.complemento}"
               placeholder="Complemento"><br>

        <button type="submit" name="logica" value="${logica}">Confirmar</button>
        <c:if test="${logica == 'AtualizarVendedor'}">
            <c:url value="vendedores?logica=ListarVendedores">
        ...
        <table>
            ...
            <c:forEach var="v" items="${vendedores}">
                <tr>
                    ...
                    <td><c:url value="vendedores?logica=EditarVendedor&id=${v.id}">Editar</c:url></td>
                    <td><c:url value="vendedores?logica=RemoverVendedor&id=${v.id}&usuario_id=${v.usuario.id}&endereco_id=${v.usuario.endereco.id}">Remover</c:url></td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
